...
MAC_perm(
  data = example_data,
  MACs_observ = MAC_example,
  num_perms = 10,
  max_lag_prop = 1 / 3,
  FDR_cutoffs = 101,
  perm_file_name = "example"
)
...
for (i in (1:380)) {
  if (lag[i] == 0) {
    ...
    lag_bin[i] <- "31-40"
  } else if (lag[i] > 40 & lag[i] <= 50) {
    lag_bin[i] <- "41-50"
  } else if (lag[i] > 50 & lag[i] <= 75) {
    lag_bin[i] <- "51-75"
  } else if (lag[i] > 75 & lag[i] <= 100) {
    lag_bin[i] <- "76-100"
  } else if (lag[i] > 100 & lag[i] <= 125) {
    lag_bin[i] <- "101-125"
  } else if (lag[i] > 125 & lag[i] <= 150) {
    lag_bin[i] <- "126-150"
  } else if (lag[i] > 151 & lag[i] <= 175) {
    lag_bin[i] <- "151-175"
  } else {
    lag_bin[i] <- ">175"
  }
...
data$lag_bin <- factor(lag_bin, levels = c(
  ">175", "151-175", "126-150", "101-125", "76-100", "51-75", "41-50", "31-40",
  "21-30", "11-20", "1-10", "0"
))
...
###################################################
library(ggplot2)
ggplot(data = data, aes(data$cors)) +
  geom_histogram(binwidth = 0.05, aes(fill = lag_bin)) +
  scale_y_sqrt() +
  xlab("Correlation") +
  ylab("Square Root Counts") +
